...
def main(k):
    ...
    for f in testing_files:
        ...
        max_d = max(k_nearest_patients, key=lambda item:item[0])[0]
        min_d = min(k_nearest_patients, key=lambda item:item[0])[0]
    ...
    name = 'clustering_figs/weighted/'+ str(k) + '_avg_closest.png'
    plt.savefig(name)
    ...
    a = ax.get_figure()
    name = 'clustering_figs/weighted/'+ str(k) + '_avg_closest_error_matrix.png'
    a.savefig(name)
