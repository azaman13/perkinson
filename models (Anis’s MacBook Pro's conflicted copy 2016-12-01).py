...
def main():
	...
	files = glob.glob("/Users/aniszaman/Dropbox/Graduate School/Fall-2016/Advanced-ML/project/src/final_data/patient_wise_data/*.csv")
	...
	for f in testing_files:
		...
		closest_model, closest_patient = find_closest_model(models, training_files, testing_A)
		pred_y = A.transpose().dot(closest_model.coef_)

		error = LA.norm(Y - pred_y)
		error_vector.append(Y - pred_y)
		print f.split('/')[-1],' patient is closest to:',closest_patient, 'pred_error',error

	err_mat = np.column_stack(error_vector)
	print err_mat
...
def find_closest_model(models, training_files, testing_A):
	...
	# should be used
	for f in training_files:
		train_A, Y = read_data(f)
		similarity = LA.norm(testing_A - train_A)
